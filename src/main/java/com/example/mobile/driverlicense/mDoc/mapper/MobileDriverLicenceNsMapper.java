...
import com.example.mobile.driverlicense.config.properties.MobileDocProperties;
...
import com.example.mobile.driverlicense.mDoc.namespace.Code;
...
import com.example.mobile.driverlicense.mDoc.utils.CborHelper;
...
public class MobileDriverLicenceNsMapper implements NamespaceDataMapperInf<DriverDetails> {
    ...
    public Map<String, DataItem> map(@NonNull DriverDetails driverDetails) {
        ...
        details.map(DriverDetails::getFamilyName).ifPresent(value -> map.put(MobileDocConstants.FAMILY_NAME, CborHelper.createDataItem(value)));
        details.map(DriverDetails::getGivenName).ifPresent(value -> map.put(MobileDocConstants.GIVEN_NAME, CborHelper.createDataItem(value)));
        ...
        details.map(DriverDetails::getIssuingCountry).ifPresent(value -> map.put(MobileDocConstants.ISSUING_COUNTRY, CborHelper.createDataItem(value)));
        details.map(DriverDetails::getIssuingAuthority).ifPresent(value -> map.put(MobileDocConstants.ISSUING_AUTHORITY, CborHelper.createDataItem(value)));
        details.map(DriverDetails::getDocumentNumber).ifPresent(value -> map.put(MobileDocConstants.DOCUMENT_NUMBER, CborHelper.createDataItem(value)));
        details.map(DriverDetails::getPortrait).ifPresent(value -> map.put(MobileDocConstants.PORTRAIT, CborHelper.createDataItem(value.getBytes())));
        ...
        details.map(DriverDetails::getHeight).ifPresent(value -> {
            if(DriverDetailsValidator.isLongValid(value)) {
                map.put(MobileDocConstants.HEIGHT, CborHelper.createDataItem(value));
            }
        });
        details.map(DriverDetails::getWeight).ifPresent(value -> {
            if(DriverDetailsValidator.isLongValid(value)) {
                map.put(MobileDocConstants.WEIGHT, CborHelper.createDataItem(value));
            }
        });

        details.map(DriverDetails::getEyeColour).ifPresent(value -> map.put(MobileDocConstants.EYE_COLOUR, CborHelper.createDataItem(value)));
        details.map(DriverDetails::getHairColour).ifPresent(value -> map.put(MobileDocConstants.HAIR_COLOUR, CborHelper.createDataItem(value)));
        details.map(DriverDetails::getBirthPlace).ifPresent(value -> map.put(MobileDocConstants.BIRTH_PLACE, CborHelper.createDataItem(value)));
        details.map(DriverDetails::getResidentAddress).ifPresent(value -> map.put(MobileDocConstants.RESIDENT_ADDRESS, CborHelper.createDataItem(value)));
        details.map(DriverDetails::getPortraitCaptureDate).ifPresent(value -> map.put(MobileDocConstants.PORTRAIT_CAPTURE_DATE, CborHelper.createDataItem(value)));

        if(mDocProperties.isAgeInYears()) {
            details.map(DriverDetails::getBirthDate).ifPresent(value -> map.put(MobileDocConstants.AGE_IN_YEARS, CborHelper.createDataItem(DurationUtils.getAge(StringParser.parseToLocalDateTime(value)))));
        }

        if(mDocProperties.isAgeBirthYear()) {
            details.map(DriverDetails::getBirthDate).ifPresent(value -> CborHelper.createDataItem(StringParser.parseToLocalDateTime(value).getYear()));
        }

        //! FIXME see 7.2.5
        details.map(DriverDetails::getBirthDate).ifPresent(value -> createAgeOverNn(map, StringParser.parseToLocalDateTime(value)));

        details.map(DriverDetails::getIssuingJurisdiction).ifPresent(value -> map.put(MobileDocConstants.ISSUING_JURISDICTION, CborHelper.createDataItem(value)));
        details.map(DriverDetails::getNationality).ifPresent(value -> map.put(MobileDocConstants.NATIONALITY, CborHelper.createDataItem(value)));
        details.map(DriverDetails::getResidentCity).ifPresent(value -> map.put(MobileDocConstants.RESIDENT_CITY, CborHelper.createDataItem(value)));
        details.map(DriverDetails::getResidentState).ifPresent(value -> map.put(MobileDocConstants.RESIDENT_STATE, CborHelper.createDataItem(value)));
        details.map(DriverDetails::getResidnentPostalCode).ifPresent(value -> map.put(MobileDocConstants.RESIDNENT_POSTAL_CODE, CborHelper.createDataItem(value)));
        details.map(DriverDetails::getResidentCountry).ifPresent(value -> map.put(MobileDocConstants.RESIDENT_COUNTRY, CborHelper.createDataItem(value)));
        ...
        details.map(DriverDetails::getFamilyNameNationalCharacter).ifPresent(value -> map.put(MobileDocConstants.FAMILY_NAME_NATIONAL_CHARACTER, CborHelper.createDataItem(value)));
        details.map(DriverDetails::getGivenNameNationalCharacter).ifPresent(value -> map.put(MobileDocConstants.GIVEN_NAME_NATIONAL_CHARACTER, CborHelper.createDataItem(value)));
        details.map(DriverDetails::getSignatureUsualMark).ifPresent(value -> map.put(MobileDocConstants.SIGNATURE_USUAL_MARK, CborHelper.createDataItem(value)));
    ...
    private DataItem buildDrivingPrivileges(List<DrivingPrivilege> drivingPrivileges) {
        ...
        for (DrivingPrivilege data : drivingPrivileges) {
            ...
            Optional.ofNullable(data).map(DrivingPrivilege::getVehicleCatagoryCode)
                    .ifPresent(value -> map.put(CborHelper.createDataItem(MobileDocConstants.VEHICLE_CATEGORY_CODE),
                            CborHelper.createDataItem(value.toString())));
            Optional.ofNullable(data).map(DrivingPrivilege::getIssueDate)
                    .ifPresent(value -> map.put(CborHelper.createDataItem(MobileDocConstants.ISSUE_DATE),
                            CborEncoderUtils.cborBuildDateTime(value)));
            Optional.ofNullable(data).map(DrivingPrivilege::getExpiryDate)
                    .ifPresent(value -> map.put(CborHelper.createDataItem(MobileDocConstants.EXPIRY_DATE),
                            CborEncoderUtils.cborBuildDateTime(value)));
            ...
            Optional.ofNullable(data).map(DrivingPrivilege::getCodes).ifPresent(codes -> {
                ...
                for (Code code : codes) {
                    ...
                    Optional.ofNullable(code).map(Code::getCode)
                            .ifPresent(value -> codeMap.put(CborHelper.createDataItem(MobileDocConstants.CODE),
                                    CborHelper.createDataItem(value.toString())));
                    Optional.ofNullable(code).map(Code::getSign)
                            .ifPresent(value -> codeMap.put(CborHelper.createDataItem(MobileDocConstants.SIGN),
                                    CborHelper.createDataItem(value.toString())));
                    Optional.ofNullable(code).map(Code::getValue)
                            .ifPresent(value -> codeMap.put(CborHelper.createDataItem(MobileDocConstants.VALUE),
                                    CborHelper.createDataItem(value.toString())));
                    codeMap.end();
                }
                DataItem codesDataItem = codesArr.end().build().get(0);
                map.put(CborHelper.createDataItem(MobileDocConstants.CODES), codesDataItem);
            });
    ...
    private Map<String, DataItem> createAgeOverNn(Map<String, DataItem> map, LocalDateTime birthdate) {
        ...
        for (Integer n : mDocProperties.getAgeOverNn()) {
            ...
            map.put(key, CborHelper.createDataItem(value));
        }
        return map;
    }
